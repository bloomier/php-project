...
(function(){
    ...
    var o={
        ...
        handler:function(){
            ...
            $('.btn-back').bind("click", function(){
                var href = __ROOT__ + "/Base/Client/index";
                if(needBack){
    ...
    var functions = {
        ...
        addUpdate: function(){
            ...
            if(name == ''){
                swal({ title: '请填写客户名称', type: 'warning', confirmButtonText: "确定"});
                return ;
            }

            var phone_num = $.trim($("#phone_num_id").val());
            if(phone_num == ''){
                swal({ title: '请填写客户电话', type: 'warning', confirmButtonText: "确定"});
                return ;
            }

            if(!setting.telephoneRegex.test(phone_num)){
                swal({ title: '请填写合法电话号码', type: 'warning', confirmButtonText: "确定"});
                return ;
            }

            var email = $.trim($("#email_id").val());
            if(email != '' && !setting.emailRegex.test(email)){
                swal({ title: '请填写合法的邮箱地址', type: 'warning', confirmButtonText: "确定"});
                return ;
            }
            ...
            $.post(__ROOT__ + "/Base/Client/addOrUpdate",param).success(function(json){
                ...
                if(json.code && json.code == 1){
                    ...
                    swal({ title: json.msg, type: 'warning', confirmButtonText: "确定"});
                }
            });
        }
    }
