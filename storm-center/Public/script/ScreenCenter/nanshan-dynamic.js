...
(function(){
        ...
        autoPlay:function(){
            ...
            if(autoPlay){
                ...
                var reloadStep=function(){
                    ...
                    var call_panels=['reload_risklevel','reload_riskarea','reload_zeroday','reload_security','reload_porttree','reload_pagelarge','reload_serverexption'];
                    $.each(call_panels,function(i,panel){
                        later(function(){
                            __reload__[panel].load.call(w);
                        },i*3000);
                ...
                later(function(){
                    reloadStep();
                    var len=__PROVINCES__.length;
                    var index=0;
                    setInterval(function(){
                        var p=__PROVINCES__[index];
                        if(index==__PROVINCES__.length){
                            location.href=location.href;
                        }else{
                            event.jump_into_province.call(o,p,true);
                            reloadStep();
                        }
                        index+=1;
                ...
                setInterval(function(){
                    $.post(__WEBROOT__ + "/ScreenCenter/GovDynamic/refresh").success(function(){
        ...
        init:function( callback){
            ...
            require(
                ...
                function (ec) {
                    ...
                    var parts=['riskLevelRank','vulsTypeRankCloud','securityTypeRank','securityAreaRank','portTreeMap'];//非中间面板初始化集合
    ...
    var event={
        jump_into_province:function(province,noStep){//跳入某一个省份
            var w=this;
            if(__functions__.isSpecialCity(province)){
                event.jump_into_city.call(w,province,noStep);
                return;
            }
    ...
    var __functions__={
        getZeroData:function(type){
            ...
            }else if(w.dataType.type=='province'||__functions__.isSpecialCity(w.dataType.data)){
        ...
        getDistLocation:function(dist){
            var w=this;
            var location;
            if($.inArray(w.dataType.data,specialCity)!=-1){
                location= dist+"_"+dist;
            }else{
                location= w.dataType.data+"_"+dist;
            }
            return location;
        },
        isSpecialCity:function(city){
            return $.inArray(city,specialCity)!=-1;
        }
    ...
    var __reload__={
        ...
        loadMapper:function(){
            var w=this;
            $.ajax({
                url:__WEBROOT__+"/ScreenCenter/GovDynamic/vulsMapper",
                async:false,
        ...
        loadOtherData:function(){
            var w=this;
            $.ajax({
                url:__WEBROOT__+"/ScreenCenter/GovDynamic/zeroDayData",
                async:false,
                success:function(json){
                    if(json.code>0){
        ...
        loadData:function(location){
            ...
            var w=this;
            $.ajax({
                url:__WEBROOT__+"/ScreenCenter/GovDynamic/getLocationData",
                data:{location:location,parent:(curProvince||w.dataType.data)},
                async:false,
                type:"POST",
                success:function(json){
                    if(json.code>0){
        ...
        reload_monitorData:function(callback){
            ...
            $.post(__ROOT__+"/ScreenCenter/NanShan/monitorData").success(function(json){
                console.info(json);
                ...
                $.each(data,function(i,d){
                    ...
                    var tr=$("<tr><td>"+newTime+"</td><td>"+name+"</td><td><span class='label "+color[d.type]+"'>"+ d.type+"</span></td></tr>");
                    tbody.append(tr);
        ...
        reload_securityEventExample:function(callback){
            ...
            wraper.html("");
            securitys.sort(function(a,b){//先把顺序打乱
                return a.domain.length- b.domain.length;
            });
            securitys.forEach(function(d){
                //var path= w.imgserver+"/upload/"+d['img'];
                var path= __PUBLIC__+"/js/nanshan/"+d['img'];
        ...
        reload_main:{
            ...
            reload_main_surveyCateGoryCycle:function(callback){
                ...
                var vuls= w.data.vuls
                options_functions.resetCycleOption(options.main_categoryCycle3,"高危风险:"+vuls.risk_rank.high.total,vuls.risk_rank.high.total,vuls.has_risk.yes);
                w.main_categoryCycle3.setOption(options.main_categoryCycle3);

                options_functions.resetCycleOption(options.main_categoryCycle4,"中危风险:"+vuls.risk_rank.mid.total,vuls.risk_rank.mid.total,vuls.has_risk.yes);
                ...
                if(area_rank.length){
                    ...
                    for(var i=0;i<area_rank.length;i++){
                        ...
                        if((data[a_rank.name])){
                            ...
                            var high = ((data[a_rank.name]['high']||0)*100/total).toFixed(2);
                            var mid = ((data[a_rank.name]['mid']||0)*100/total).toFixed(2);
                            var low = ((data[a_rank.name]['low']||0)*100/total).toFixed(2);
                            if(isNaN(high)){
                                high = "0.00";
                            }
                            if(isNaN(mid)){
        ...
        reload_riskarea:{
            ...
            reload_riskarea_properties:function(vulsId,callback){
                ...
                $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/vulsDescAndAdvice",{vulsId:vulsId}).success(function(json){
        ...
        reload_zeroday:{
            ...
            reload_zeroday_properties:function(type,callback){
                ...
                $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/zeroDayDesc",{type:type}).success(function(json){
        ...
        reload_porttree:{
            ...
            reload_porttree_properties:function(port,callback){
                ...
                $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/portDesc",{port:port}).success(function(json){
        ...
        reload_report:{
            load:function(reportParam,callback){
                ...
                $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/report",reportParam).success(function(json){
            ...
            loadSecurity:function(json,callback){
                ...
                $('.report-event-deal-state',wraper).html(event.deal_state==3?"已修复":"未修复");
    ...
    var __load_dist__={
        load_serverexption:function(dist){
            ...
            var location=__functions__.getDistLocation.call(w,dist);
            $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/loadDistData",{location:location,type:'websurvey'}).success(function(json){
                    if(!json.data){
                        event.jump_out_dist();
                        return;
                    }
        ...
        load_security:function(dist){
            ...
            var location=__functions__.getDistLocation.call(w,dist);
            $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/loadDistData",{location:location,type:'security'}).success(function(json){
                    if(!json.data){
                        event.jump_out_dist();
                        return;
                    }
        ...
        load_porttree:function(dist){
            ...
            $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/loadDistData",{location:location,type:'port'}).success(function(json){
        ...
        load_riskarea:function(dist){
            ...
            var location=__functions__.getDistLocation.call(w,dist);
            $.post(__WEBROOT__+"/ScreenCenter/GovDynamic/loadDistData",{location:location,type:'vuls'}).success(function(json){
                if(!json.data){
                    event.jump_out_dist();
                    return;
                }
    ...
    var handler={
        ...
        echarts_steps_event:function(ecConfig){//echart 的面板切换的事件
            ...
            var events={
                ...
                portTreeMap:function(param){
                    __reload__.reload_porttree.load.call(w,param.name,function(){
                        __functions__.stepinto.call(w,stepNames.porttree);
                    });
                },
