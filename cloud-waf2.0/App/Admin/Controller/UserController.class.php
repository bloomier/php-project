...
//        $param['roles']=$roles;
//        if(!I("_id")) {//添加
//            //change_db_main();//判断用户名是否已经存在（必须保证外面的映射表用户名数据唯一）
//            $md=new StringModel(Constants::$DB_AUTH_USER);
//            $exist=$md->where(array(username=>I("username")))->find();
//            if($exist){
//                $this->ajaxReturn(array("code"=>0,"msg"=>"添加失败失败,已经存在用户名"));
//            }
...
//            $param["password"]=md5($randPwd);
//            $row=$md->add($param);
//            $ret=array("code"=>1,"msg"=>"添加成功用户成功,该用户的初始密码:".$randPwd);
//            $ret['item']=$param;
//            if(!$row){
//                $ret['msg']="添加失败";
//            }
//            $ret['randPwd']=$randPwd;
//            $this->addRegionRelation();
//            $this->ajaxReturn($ret);
//        }else{//修改
//            $param["_id"]=intval(I("_id"));
//            unset($param['username']);
...
//        $md=new AutoIncrementModel(Constants::$DB_AUTH_USER);
//        $row=$md->field("_id,username,name,roles,email,phone,is_manager,data_config")->select();
