...
(function(){
    ...
    var init_view = {
        init_table:function(){
            ...
            w.table=$("#waf_site_table").DataTable($.extend(_dataTable_setting._server(),{
                ajax:{
                    url: __WEBROOT__ + "/Home/Sites/pageSite",
                    type:"post",
                    dataSrc:"items",
                    data: function ( d ) {
                        //$.extend(d,$('#extra').val());
                ...
                rowCallback:function( row, data, index ){
                    ...
                    var reportBtn=$('<a class="btn-report btn btn-xs btn-info " target="_blank" style="algin-left: 2px;" href="'+location+'" title="查看报告"><i class="fa fa-file-text"></i>&nbsp;报告</a>');
                    ...
                    if(data['relation_type'] == 1){
                        ...
                        $("td:eq(0)",row).html($("<input type='checkbox' class='i-checks' name='one'/>"));
                    } else {
    ...
    var __functions__ = {
        ...
        getAllKindsOfCount: function(){
            $.post(__ROOT__+"/Home/Sites/getAllKindsOfCount",null).success(function(json){
                if(json){
                    $.each(json, function(point, item){
                        if(point.indexOf('Count') != -1){
                            $("." + point).text(item);
                        } else {
                            allKindsOfSite[point] = item;
                        }
                    });
                }
            });
    ...
    var init_btn = {
        init_bind : function(){
            ...
            $(".font-num").bind("click", function(){
                ...
                if($(this).html() != "0"){
                    var str = array[0];
                    str = str.replaceAll('Count','Sites');
                    str = allKindsOfSite[str];
                    $("#clickWhichCountId").val(str);
                    if($(this).text() != '0'){
                        w.table.draw();
                    }
                }
