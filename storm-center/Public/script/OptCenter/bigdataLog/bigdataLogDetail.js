...
(function(){
    ...
    var __init__ = {
        getDataByRowkey: function(rowkey){
            ...
            $.post(__ROOT__ + "/OptCenter/BigdataLog/search_one", param).success(function(json){
                json = $.parseJSON(json);
                console.info(json);
                ...
                for(var key in json.other) {
                    ...
                    if(json.other[key] != ''){
                        if(arr.indexOf(key) == -1){//数组中不存在
                            ...
                            if(key != 'severity'){
                                $("#"+ key + "_id").html(json.other[key]);
                            } else {
                                // 事件级别需要转换显示
                                var severity = json.other[key];
                                var color = "gray";
                                ...
                                var severityState = $("<button type='button' class='btn " + color + " btn-sm' disabled='disabled' style='width: 80px;height: 20px;line-height:5px;'>" + severity + "</button>");
