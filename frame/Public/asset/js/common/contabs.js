...
$(function(){
    ...
    function c(){
        ...
        //遍历所有title上的Tabs（已经打开过的菜单）
        $(".J_menuTab").each(function(){
            //已经打开过的菜单中存在当前点击的左边菜单
            if($(this).data("id")==o){
                //如果不是当前正在显示的菜单，将该菜单对应tab状态置为active，其他去除active属性
    ...
    //在IFrame中新增Tab
    addOneTab = function(url,title,num){
        var id = url;
        if(url.indexOf("?") != -1){
            id = url.split("?")[0];
        }
        ...
        if(flag){
            var tab = '<a href="javascript:;" class="active J_menuTab" data-id="' + id + '">' + title + ' <i class="fa fa-times-circle"></i></a>';
            $(".J_menuTab", parent.document).removeClass("active");
            var form = '<iframe class="J_iframe" name="iframe' + num + '" width="100%" height="100%" src="' + url + '" frameborder="0" data-id="' + id + '" seamless></iframe>';
            //$('#btnOk', parent.document)
            $(".J_mainContent", parent.document).find("iframe.J_iframe").hide().parents(".J_mainContent", parent.document).append(form);
            $(".J_menuTabs .page-tabs-content", parent.document).append(tab);
    ...
    // 在IFrame中删除当前某个tab
    colseCurrentTab = function(url){
        if(url.indexOf("?") != -1){
            url =  url.split("?")[0];
        }

        var i = 0;
        $(".page-tabs-content", parent.document).children("[data-id]").not(":first").each(function(){
            //tabs.push($(this));
            if($(this).data("id") == url){
                var w = $(".page-tabs-content", parent.document).children("[data-id]").eq(i);
                w.addClass("active");
                $('.J_iframe[data-id="'+ w.data("id") +'"]', parent.document).show();
                $('.J_iframe[data-id="'+$(this).data("id")+'"]', parent.document).remove();
                $(this).remove();
                return false;
            }
            i++;
