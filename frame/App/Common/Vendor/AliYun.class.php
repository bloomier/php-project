...
class AliYun  {
    ...
    /**  API 版本号 */
    public static $VERSION = "2016-04-10";
    /** 阿里云颁发给用户的访问服务所用的密钥ID */
    public static $ACCESS_KEY_ID = "32wcLIC9vYxhZ4eY";
    /** 签名方式，目前支持HMAC- SHA1 */
    public static $SIGNATURE_METHOD = "HMAC-SHA1";
    /** 签名算法版本，目前版本是1.0 */
    public static $SIGNATURE_VERSION = "1.0";
    ...
    /** 5、 配置高防实例域名防护配置 */
    public function  CreateDomain($config){
        $params = array();
        $params = $this->createCommomParams($params);
...
//        $params['CcEnable'] = 1;//是否开启cc防护，0：否，1：是，默认是
    ...
    /** 6、 删除高防实例域名防护配置,通过实例id和域名删除（InstanceId，Domain） */
    public function DeleteDomain($config){
        $params = array();
        $params = $this->createCommomParams($params);
...
//        $params['PageNumber'] = "10"; //页大小，默认为20，最大50
    ...
    public function createCommomParams($params){
        $params['Format'] = AliYun::$FORMAT;//返回值的类型，支持JSON与XML 。默认为XML
        $params['Version'] = AliYun::$VERSION;//API 版本号
        $params['AccessKeyId'] = AliYun::$ACCESS_KEY_ID;//阿里云颁发给用户的访问服务所用的密钥ID
        $params['SignatureMethod'] = AliYun::$SIGNATURE_METHOD;//签名方式，目前支持HMAC- SHA1
        $params['Timestamp'] = date("Y-m-d\TH:i:s\Z",time() - 8 * 3600);//请求的时间戳。 并需要使用UTC 时间。格式为：YYYY- MM- DDThh:mm:ssZ
        $params['SignatureVersion'] = AliYun::$SIGNATURE_VERSION;//签名算法版本，目前版本是1.0
