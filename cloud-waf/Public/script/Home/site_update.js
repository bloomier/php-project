...
(function(){
    ...
    var o={
        ...
        //设置当前站点信息
        setCurrentSiteValue : function(){
            var w = this;
            var currentSite = $.parseJSON(decodeURIComponent($("#currentSiteId").val()));
            setting.currentSite = currentSite;

            $("#siteName_id").val(currentSite.name);
            $("#ip_id").val(currentSite.ip);
            $("#port_id").val(currentSite.port);
            $("#domains_id").val(currentSite.domain);

            var is_http = currentSite.is_http;
            if(is_http == 1){
                functions.onSwitchBtton('is_http');
                $(".is_show_http_div").show();
            } else {
                functions.offSwitchBtton('is_http');
                $(".is_show_http_div").hide();
            }

            $("#file1").val(currentSite.cert_public);
            $("file2").val(currentSite.cert_private);
            $("#file3").val(currentSite.cert_ca_link);

            //设置防护模式
            var protect_mode = currentSite.zone_rule.rule_engine;
            var lis = $("ul li.protect_mode");
            for(var i = 0,num = lis.length; i < num; i++){
                var temp = $(lis[i]);
                if(temp.children().attr("data") == protect_mode){
            ...
            var is_ddos = currentSite.is_ddos;
            if(is_ddos == 1){
                functions.onSwitchBtton('is_ddos');
                $(".is_show_ddos_div").show();
            } else {
                functions.offSwitchBtton('is_ddos');
                $(".is_show_ddos_div").hide();
            }
            //
            var is_waf_warning = currentSite.is_waf_warning;
            if(is_waf_warning == 1){
                functions.onSwitchBtton('is_waf_warning');
            } else {
                functions.offSwitchBtton('is_waf_warning');
            }


            $("#highInstance_id").val(currentSite.InstanceId);

            var is_loophole_scan = currentSite.is_loophole_scan;
            if(is_loophole_scan == 1){
                functions.onSwitchBtton('is_loophole_scan');
            } else {
                functions.offSwitchBtton('is_loophole_scan');
            }
            //
            var loophole_warning = currentSite.loophole_warning;
            if(loophole_warning == 1){
                functions.onSwitchBtton('loophole_warning');
            } else {
                functions.offSwitchBtton('loophole_warning');
            }
            //
            var is_available = currentSite.is_available;
            if(is_available == 1){
                functions.onSwitchBtton('is_available');
            } else {
                functions.offSwitchBtton('is_available');
            }
            //
            var available_warning = currentSite.available_warning;
            if(available_warning == 1){
                functions.onSwitchBtton('available_warning');
            } else {
                functions.offSwitchBtton('available_warning');
            }

            //console.info(currentSite.warnContacts);
            if(currentSite.warnContacts){
                //已选择联系人
                setting.userAddList = currentSite.warnContacts;
            }
        ...
        initTable: function(){
            ...
            addUserTable = $("#add_users_table").DataTable($.extend(_dataTable_setting._server(),{
                columns: [
    ...
    var functions = {
        //开启开关按钮
        onSwitchBtton: function(classType){
            $("." + classType).addClass('on');
            $("." + classType).children('span.text').text('启用');
        },
        //关闭开关按钮
        offSwitchBtton: function(classType){
            $("." + classType).removeClass('on');
            $("." + classType).children('span.text').text('禁用');
        },
        //修改站点
        updateSite: function(){
            ...
            if(domains.indexOf('*') != -1){
                storm.alertMsg('站点域名中不能包含*符号','warning');
                return ;
            }

            if(domains.length > 64){
                storm.alertMsg('站点域名过长','warning');
                return ;
            }
            ...
            var _id = setting.currentSite._id;
            var oldDomain = setting.currentSite.domain;
            var oldInstanceId = setting.currentSite.InstanceId;
            var id = setting.currentSite.id;
            var old_is_ddos = setting.currentSite.is_ddos;
            ...
            $.post(__ROOT__ + "/Home/Sites/updateSite",param).success(function(json){
