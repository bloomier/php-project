...
class ClientController extends BaseController {
    ...
    public function getcurrentObject(){
        $_id = I('_id');
        if($_id){
            $md = new StringModel(Constants::$DB_CLIENT_LIST);
    ...
    public function addOrUpdate(){
        ...
        if(I('state') == "0"){
            ...
            if($rows){
                $this->ajaxReturn(array(code=>0,msg=>"该客户关联了有效合同信息不能置为无效状态"));
            }
        }
        $params['desc'] = I('desc');
        if(I('_id')){
    ...
    public function delete(){
        ...
        $cWhere['state'] = 1;
        $cWhere['end_date'] = array('$gte'=>date("Y-m-d"));
        $cWhere["client"]=array("in",$_ids);
        $rows = $cMd->where($cWhere)->select();
        if($rows){
            $this->ajaxReturn(array(code=>0,msg=>"客户关联了合同信息不能删除"));
        }
        $md=new StringModel(Constants::$DB_CLIENT_LIST);
