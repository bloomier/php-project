    ...
    var __bind__ = {
        bindFunction:function(){
            ...
            $(".all-left").bind("click", function(){
                ...
                __init__.initTree("srcTree", targetNode, dest_setting);
            });

            /**
             * 更新
             */
            $(".add-policy-group").bind("click", function(){

                var enable = $('input[name="policy-group-enable"]:checked').val();// 获取radio的值
                var name = $(".policy-group-name").val();// 获取名称
                var desc = $.trim($(".policy-group-desc").val());// 获取备注
                var dest = $.fn.zTree.getZTreeObj("destTree");// 获取目标列表
