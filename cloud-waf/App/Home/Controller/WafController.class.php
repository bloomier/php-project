...
class WafController extends BaseController {
    ...
    public function flowRank(){
        ...
        $json=http_post(C("API_PATH")."/api/cloudwaf/domainflowout/list",$param,'json');
        $lasest=array();
        $all=array();
    ...
    public function visitCountRank(){
        $json=http_post(C("API_PATH")."/api/cloudwaf/domainvisit/list",array("point"=>10),'json');
        $lasest=array();
         $all=array();
        foreach($json['data']['all'] as $k=>$v){
             $all[]=array(name=>$k,value=>$v);
    ...
    public function attackCountRank(){
        $json=http_post(C("API_PATH")."/api/cloudwaf/domainattack/list",array("point"=>10),'json');
        $lasest=array();
        $all=array();
        foreach($json['data']['all'] as $k=>$v){
            $all[]=array(name=>$k,value=>$v);
    ...
    public function ipCountRank(){
        ...
        $json=http_post(C("API_PATH")."/api/cloudwaf/visitip/realtimeTopN",$param,'json');
        $visitIP = array();
        $attackIP = array();
        ...
        $json=http_post(C("API_PATH")."/api/cloudwaf/attackip/realtimeTopN",$param,'json');
        foreach($json['items'] as $k=>$v){
    ...
    public function visitAttackCounterList(){
        $json=http_post(C("API_PATH")."/api/cloudwaf/visit_attack/countlist",null,'json');
    ...
    public function flowNormalCounterList(){
        $json=http_post(C("API_PATH")."/api/cloudwaf/flow/countlist",null,'json');
       //dump($json['data']);
        $this->ajaxReturn($json['data']);
    }
    //引擎IP
...
//        dump(I("ipList"));
    ...
    public function attackTimes(){
        $json=http_post(C("API_PATH")."/api/cloudwaf/defense/count",null,'json');
        $this->ajaxReturn($json['data']);
...
//        $json=http_post(C("SOC_PATH")."/host/network/g20/secs",array("deviceId"=>"destHostName_g20.org"),'json');
