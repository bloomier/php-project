...
(function(){
    ...
    var o = {
        ...
        initCommon:function(){
            console.info(value);
            if(value && value != ''){
                var siteDoc = value.domainInfo;
                $(".title", $(".common-info")).html(siteDoc['title']);
                $(".ip", $(".common-info")).html(siteDoc['ip']);
                $(".isp", $(".common-info")).html(siteDoc['isp']);
                $(".registration_email", $(".common-info")).html(siteDoc['register_email']);
                if(siteDoc['domain'].length > 20){
                    $(".domain", $(".common-info")).html('<abbr title="' + siteDoc['domain'] + '">' + siteDoc['domain'].substr(0,20) +'...</abbr>');
                }else{
                    $(".domain", $(".common-info")).html(siteDoc['domain']);
                }
                $(".region", $(".common-info")).html(siteDoc['province'] + siteDoc['city']);
                $(".vailTime", $(".common-info")).html(siteDoc['expire_time']);
                $(".registration_name", $(".common-info")).html(siteDoc['registration_name']);
            }
        },
        initSummary:function(){
            if(value && value != ''){
                ...
                if(currentDateDoc && currentDateDoc.statistics && currentDateDoc.statistics.okNodeNum && currentDateDoc.statistics.nodeNum){
                    access_num = currentDateDoc ? (currentDateDoc.statistics.okNodeNum + "/" + currentDateDoc.statistics.nodeNum) : 0;
                }

                var max_connect_time = 0;
                if(currentDateDoc && currentDateDoc.statistic && currentDateDoc.statistics.maxConectTime){
                    max_connect_time = currentDateDoc.statistics.maxConectTime;
                }

                var month_error_time = 0;
                if(monthDoc&&monthDoc.offline && monthDoc.offline.all){
                    month_error_time = Math.round(monthDoc.offline.all.disconnect_time / 1000 / 60);
                }

                var month_update_times=0;
                if(monthDoc&&monthDoc.update_rate){
                    month_update_times = monthDoc.update_rate.update_count;
                }

                var last_update_time = '未更新';
                if(monthDoc && monthDoc.update_rate){
                    last_update_time = monthDoc.update_rate.last_update_time||"未更新";
                }

                var day_error_time = 0;
                if(currentDateDoc && currentDateDoc.offline && currentDateDoc.offline.all){
                    day_error_time = Math.round(currentDateDoc.offline.all.disconnect_time/1000/60);
                }

                var infos={
                    day_error_time:day_error_time,//currentDateDoc ? (currentDateDoc.offline.all ? Math.round(currentDateDoc.offline.all.disconnect_time/1000/60) : 0) : 0,
                    month_error_time: month_error_time,
                    home_load_time: home_load_time,
                    access_num:access_num,
                    max_connect_time:max_connect_time,
                    month_update_times:month_update_times,
        ...
        initDraw: function(){
            ...
            require(
                ...
                function (ec) {
                    ...
                    if(value && value['resourceDoc'] && value['resourceDoc']['homepage_data']){
                    ...
                    if(value && value.monthDoc&&value.monthDoc.update_rate.last_update_time){
        ...
        initMapChartTimeChanged:function(json){
            ...
            w.mapChart.on('timelineChanged', function(a,b){
                ...
                //先全部行初始化
                if(b.component.timeline.options[a.currentIndex].series){
        ...
        initOption: function(){
            ...
            w.chartOption = function(json){
                var monthDoc;
                if(json && json.monthDoc){
                    monthDoc = json.monthDoc
                }
            ...
            w.faultBarOption = function(json){
                ...
                var offline = lastDate['offline'];
                if(!lastDate['offline']){
                    $.each(json['node'], function(p, v){
                        option.xAxis[0].data.push(v['cn_name']);
                        var connect_time = 0;
                        var connect_count = 0;
        ...
        getMapCharData:function(json){
            ...
            if(json && json.dateDoc && json.dateDoc[json.dateDoc.length-1]){
        ...
        getOneValue: function(node,hour,dateDoc,provinceName){
            ...
            if(datas){
                if(datas[node]&&datas[node][hour]){
                    ...
                    //Math.ceil(data.avg_connect_time+data.avg_nslookup_time);
                    var num = Math.ceil(data.avg_connect_time+data.avg_nslookup_time);
