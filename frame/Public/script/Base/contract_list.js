        ...
        init_table:function(){
            ...
            w.table=$("#contract_site_table").DataTable($.extend(_dataTable_setting._static(),{
                ...
                rowCallback:function( row, data, index ){
                    ...
                    $("td:eq(9)",row).html("");
                    //btn-attention
                    var editBtn=$('<a href="javascript:void(0)" class="btn btn-xs btn-info" title="修改" style="margin-left: 4px;"><i class="fa fa-edit"></i> 修改</a>');
                    editBtn.bind("click",function(){
                        __functions__.update_contract.call(w,data);
                    });

                    var location=__ROOT__+"/Home/Sites/addSite/contract_id/"+data['_id'];
                    var addSite=$('<a class="btn btn btn-xs btn-primary" href="' + location + '" title="添加站点" style="margin-left: 4px;"><i class="fa fa-plus"></i> 添加站点</a>');

                    addSite.bind("click", function(){
                        //var href = __ROOT__ + "/Home/Sites/addSite";
                        addOneTab(location, '新增站点', 10);
                    });



                    var detailBtn = $('<a class="btn btn-xs btn-success" href="#" title="详情"><i class="fa fa-info-circle"></i> 详情</a>');
                    detailBtn.bind("click", function(){
                        var detailLocation = __ROOT__+"/Base/Contract/contractDetail/contract_id/"+data['_id'];
                        //console.info(detailLocation);
                        addOneTab(detailLocation, '合同详情', 11);
                    });
                ...
                initComplete:function(){
                    var addBtn=$('<a  href="#" class="btn-add btn btn-sm btn-primary" ><i class="fa fa-plus"></i> 添加合同</a>');
                    addBtn.bind("click", function(){
                        w.dialog.setTitle("<h3>添加合同</h3>");
                        w.dialog.open();
                    });
                    var deleteBtn=$('<a  href="#" class="btn-delete-all btn btn-sm btn-danger" ><i class="fa fa-minus"></i> 终止合同</a>');
                    deleteBtn.bind("click", function(){
                        __functions__.stopContract.call(w);
                    });
