...
(function(){
    var __functions__={
        getHourKeys:function(){
            ...
            var arr=[];
            for(var i=0;i<24;i++){
                var t= ($("#currentDateKey").val())+" "+(i>=10?(""+i):("0"+i))+":00:00";
                arr.push(t);
            }
            return arr;
        },
    ...
    monitor={
        ...
        loadData:function(domain,dateKey,callback){
            ...
            var monitorDataValue = $.parseJSON($("#monitor_data").text());
            if(monitorDataValue){
                w.monitorData = monitorDataValue.data;
                callback&&callback();
            }else{
                //domain = 'www.dbappsecurity.com.cn';
                $.ajax({
                    url:__WEBROOT__+"/Home/DailyReport/monitorData",
        ...
        view:{
            load:function(){
                ...
                $('#date_deal_map').height(h *.6);
                $('#date_deal_bar').height(h);
                $('#gantt').height(h);

                $('#cdn_map').height(h *.5);
                $('#resource_pie').height(h *.4);
                $('#resource_leida').height(h *.4);
                $('#update_line').height(h *.5);
            ...
            date_deal_map:function(){
                ...
                $.each(hourKeys,function(i,hourKey){
                    ...
                    monitor.options.date_deal_map.options.push({
                        title : {
                            'text':hourKey.slice(0, 13)+'时服务质量概况',
                            x: 'center',
                            textStyle: {
                                fontSize: 16,
                                fontFamily: '华文细黑',
            ...
            date_deal_bar:function(hourKey){
                ...
                monitor.options.date_deal_bar.yAxis[0].data=nodes_tmp;
            ...
            offline_line_record:function(){
                ...
                $(".offline-record-table").DataTable(_dataTable_setting._report());
            ...
            cdn_table:function(){
                ...
                $(".cnd-table").DataTable(_dataTable_setting._report());
            ...
            resource_table:function(){
                ...
                if(w.monitorData.resource&&w.monitorData.resource.resource){
                    ...
                    $(".resource-table").DataTable(_dataTable_setting._report());
                }
            ...
            warning_table:function(){
                ...
                if(w.monitorData.warning){
                    ...
                    $(".warning_table").DataTable(_dataTable_setting._report());
                }
            }
