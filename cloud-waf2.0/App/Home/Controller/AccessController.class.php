...
class AccessController extends BaseController {
    ...
    public function addOrUpdateAccess(){
        ...
        $config = $this->createConfig(I(),$uuid,true);
        $params = $this->createConfig(I(),$uuid,false);;
        if(!I("ip")){
    ...
    /** 组装数据 */
    private function createConfig($param, $uuid, $isEncode){
        $config = array();
        $config['action'] = $param['action'];
        $config['expire'] = $param['expire'];
        $config['uuid'] = $uuid;
        $sip = array();
        //www.hzsj.gov.cn 183.136.190.62
        $sip[] = array($param['ip'],$param['forwarded']);
        $domain = explode(',', $param['domains']);
        $url = explode(',', $param['urls']);
        $match = array();
        $match["sip"] = $sip;
        $match["domain"] = $domain;
        $match["url"] = $url;
        if($isEncode){
            ...
            $config['ip'] = $param['ip'];
            $config['createModifyTime'] = date("Y-m-d");
            $config['_id'] = $uuid;
        }
